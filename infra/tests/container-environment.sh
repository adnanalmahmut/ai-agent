...
# into an assertion about the runner rather than about the deployment
# composition.
#
...
unset_names=$(
  {
    ...
    grep -hoE '\$\{[A-Z][A-Z0-9_]*' infra/compose/compose.yaml \
      infra/compose/compose.deploy.yaml | sed 's/^\${//'
  } | sort -u
)
unset_fixture=''
...
assert_jq 'backend gets blank APP_ORIGIN_PUBLIC' '.services.backend.environment.APP_ORIGIN_PUBLIC == ""'
assert_jq 'backend gets blank APP_ORIGIN_APP' '.services.backend.environment.APP_ORIGIN_APP == ""'
assert_jq 'backend gets blank APP_ORIGIN_ADMIN' '.services.backend.environment.APP_ORIGIN_ADMIN == ""'
assert_jq 'backend gets blank APP_ORIGIN_API' '.services.backend.environment.APP_ORIGIN_API == ""'
