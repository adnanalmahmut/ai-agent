...
function Sidebar({
  ...
  /**
   * Accessible name and description of the mobile drawer.
   *
   * Radix's Dialog requires both, and on mobile this sidebar *is* a Dialog —
   * so these strings are read aloud to a screen-reader user. They are props
   * rather than literals because this package has no dictionary and must
   * never grow one: the application supplies them already translated, the
   * same way `DirectionProvider` receives a direction rather than a locale.
   *
   * The English defaults keep every existing call site working unchanged.
   */
  mobileTitle?: string
...
function SidebarTrigger({
   ...
   *
   * A prop rather than a literal for the same reason as `Sidebar`'s
   * `mobileTitle`: this package has no dictionary and must never grow one, so
   * the application supplies the string already translated. The English
...
function SidebarMenuButton({
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent
        // The collapsed rail sits on the reading-start edge, so its tooltip
        // belongs on the end edge — which is the left in Arabic. Radix takes a
        // physical side, so this is one of the few places direction has to be
        // read rather than expressed logically in CSS.
...
function SidebarMenuSkeleton({
  ...
  const width = `${50 + Array.from(id).reduce((sum, character) => sum + character.charCodeAt(0), 0) % 40}%`
