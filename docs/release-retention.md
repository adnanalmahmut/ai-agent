...
Each record must contain a valid source SHA and a digest for every component in
`infra/release/components`. Records written as a component list
(`recordVersion: 1`) and the flat records older bundles wrote are both read, and
both strictly: a record missing a component protects nothing less. Retention
considers only the repositories that catalog names — today `backend`,
...
2. Enumerate images from the allowed repositories.
...
preflight remains the hard gate. Retention remains current at host bundle 16; see
