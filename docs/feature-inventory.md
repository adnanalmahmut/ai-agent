...
| Organization agents | Authorized code-owned agent catalog plus one installation per organization/agent, with definition-validated configuration, immutable enabled/configuration versions, transactional active-pointer switching, optimistic concurrency, paged history, and run acceptance/execution pinned to the selected immutable version | `apps/backend/src/agents/organization-agent-installation*`, `apps/backend/prisma/schema.prisma` |
| Content ideas | `content-idea@1`, the first production agent: an organization-scoped asynchronous request grounded in the organization's own knowledge spaces, gated by `agents.enabled`, `content_ideas.enabled`, and its installed/enabled organization-agent state, bounded by `agents.max_concurrent_runs_per_organization`, with a required idempotency key bound to the request body and a polling status/result surface | `apps/backend/src/agents/definitions/`, `apps/backend/src/content-ideas/`, `apps/platform/src/features/organization/blocks/organization-content-ideas-block.tsx` |
