...
| Knowledge | Organization-owned spaces, documents, and embedded chunks in PostgreSQL with pgvector, retrieved by exact cosine search scoped to the organization and to the granted spaces inside the ranking query itself, bounded by an operator-owned chunk ceiling; no ingestion pipeline or management surface yet | `apps/backend/src/knowledge/`, `apps/backend/prisma/schema.prisma` |
