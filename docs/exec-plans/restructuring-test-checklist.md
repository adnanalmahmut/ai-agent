...
| **An abandoned session is swept without any client request, and releases the organization's in-flight ceiling** | reconciler unit spec only, against a fake `AgentRunService` | no database-level proof, and the capacity consequence untested anywhere | **5 tests** in `test/e2e/ai/agent-run-reconciliation.e2e-spec.ts` — "an abandoned MCP session" | `--testPathPatterns 'agent-run-reconciliation'` |
